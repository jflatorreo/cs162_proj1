...
public class KThread {
	...
	public void fork() {
		...
		tcb.start(newRunnable() { //Start TCB with this thread
			public void run() {
				runThread();
			}
		});
	...
	public void join() {
		...
		if (this != currentThread && this.status != statusFinished && this.isJoined != true) {
			...
			ThreadState currState = PriorityScheduler.getThreadState(currentThread);
			PriorityScheduler.getThreadState(this).setEffectivePriority(currState);
		}
		Machine.interrupt().enable();
	}
