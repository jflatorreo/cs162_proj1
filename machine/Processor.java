...
public final class Processor {
    ...
    public Processor(Privilege privilege, int numPhysPages) {
        ...
        usingTLB = (clsVMKernel != null && clsVMKernel.isAssignableFrom(clsKernel));
     ...
     */
    private void delayedLoad(int nextLoadTarget, int nextLoadValue, int nextLoadMask) {
    ...
    private class Instruction {
        ...
        private void fetch() throws MipsException {
            if ((Lib.test(dbgDisassemble) && !Lib.test(dbgProcessor)) || Lib.test(dbgFullDisassemble))
                ...
                switch (args[i]) {
                    case Mips.RS:
                        ...
                        if (Lib.test(dbgFullDisassemble)) {
                            System.out.print("#0x" + Lib.toHexString(registers[rs]));
                            minCharsPrinted += 11;
                            maxCharsPrinted += 11;
                        }
                        break;
                    case Mips.RT:
                        ...
                        if (Lib.test(dbgFullDisassemble) && (i!=0 || !test(Mips.DST)) && !test(Mips.DELAYEDLOAD)) {
                            System.out.print("#0x" + Lib.toHexString(registers[rt]));
                            minCharsPrinted += 11;
                            maxCharsPrinted += 11;
                        }
                        break;
